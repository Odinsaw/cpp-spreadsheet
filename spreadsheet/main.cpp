...
int main() {
    ...
    {
        auto sheet = CreateSheet();
        sheet->SetCell("A1"_pos, "=B2+A2");
        sheet->SetCell("A2"_pos, "=1+2");
        sheet->SetCell("A3"_pos, "4");
        sheet->SetCell("B1"_pos, "hhh");
        sheet->SetCell("B2"_pos, "4");
        sheet->SetCell("B3"_pos, "7");
        sheet->SetCell("C1"_pos, "=B2+A2");
        sheet->SetCell("C2"_pos, "hhh");
        sheet->SetCell("C3"_pos, "=A3+B3");
        std::cout << '\n';
        sheet->PrintTexts(std::cout);
        std::cout << '\n';
        sheet->PrintValues(std::cout);
        std::cout << '\n';
        std::cout << sheet->GetPrintableSize().cols << '\t' << sheet->GetPrintableSize().rows << '\n';
        std::cout << '\n';

        sheet->SetCell("B2"_pos, "");
        sheet->SetCell("A1"_pos, "");
        sheet->SetCell("A2"_pos, "");
        sheet->SetCell("A3"_pos, "");
        std::cout << '\n';
        sheet->PrintTexts(std::cout);
        std::cout << '\n';
        sheet->PrintValues(std::cout);
